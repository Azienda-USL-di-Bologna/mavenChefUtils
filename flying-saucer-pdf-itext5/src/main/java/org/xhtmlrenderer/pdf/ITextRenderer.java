...
public class ITextRenderer {
    ...
    public ITextRenderer() {
        this(DEFAULT_DOTS_PER_POINT, DEFAULT_DOTS_PER_PIXEL, null);
    }

    public ITextRenderer(float dotsPerPoint, int dotsPerPixel, ITextUserAgent userAgent) {
        _dotsPerPoint = dotsPerPoint;

        _outputDevice = new ITextOutputDevice(_dotsPerPoint);

        if (userAgent == null) {
            userAgent = new ITextUserAgent(_outputDevice);
        }
    ...
    public void createPDF(OutputStream os) throws DocumentException, IOException {
        createPDF(os, true, 0, false, null);
    }
    
    public void createPDF(OutputStream os, boolean pdfa, PdfAConformanceLevel conformanceLevel) throws DocumentException, IOException {
        createPDF(os, true, 0, pdfa, conformanceLevel);
    }
    ...
    public void createPDF(OutputStream os, boolean finish) throws DocumentException, IOException {
        createPDF(os, finish, 0, false, null);
    }
    
      public void createPDF(OutputStream os, boolean finish, boolean pdfa, PdfAConformanceLevel conformanceLevel) throws DocumentException, IOException {
        createPDF(os, finish, 0, pdfa, conformanceLevel);
    }
     ...
     * @throws IOException
     */
    public void createPDF(OutputStream os, boolean finish, int initialPageNo, boolean pdfa, PdfAConformanceLevel conformanceLevel) throws DocumentException, IOException {
        List pages = _root.getLayer().getPages();
...
//                firstPage.getHeight(c) / _dotsPerPoint);
        ...
        if (pdfa) {
                writer = PdfAWriter.getInstance(doc, os, conformanceLevel);
        }
        else {
